...
const cart = {
    ...
    renderCart() {
        ...
        this.cartGoods.forEach(({name, id, price, count}) => {
            ...
            <td>${count * price}</td>
            <td><button class="cart-btn-delete" data-id="${id}">-</button></td>
            `;
            cartTableGoods.append(trGood);
        });
        const totalPrice = this.cartGoods.reduce((sum, good) => sum + good.price * good.count, 0);
        cartTableTotal.textContent = `${totalPrice}\$`;
        cartCount.textContent = this.cartGoods.reduce((sum, good) => sum + good.count, 0);
    },
    plusGood(id) {
...
function createCard(objCard) {
    ...
    <div class="goods-card">
        ...
        <h3 class="good-title">${objCard.name}</h3>
